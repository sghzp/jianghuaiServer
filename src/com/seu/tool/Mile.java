...
public class Mile {
	...
	public static double PreMile(String ProjectTaskID) {
		double mile = 0;
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String sql = "select mile from tb_experimentdata where PROJECT_TASK_ID = '" + ProjectTaskID + "'order by time desc limit 1";
//System.out.println(sql);
	...
	public static double PreMile(int orderid){
		String sql = "select totalmileage from tb_ordermileage where orderid = " + orderid;
		DBO dbo = new DBO();
		double mile = 0.0;
		ResultSet rs = null;
		try {
			rs = dbo.query(sql);
			while (rs.next()){
				mile = rs.getDouble("totalmileage");
			}
		}catch (Exception e){
			e.printStackTrace();
		}finally {
